...
void
Url::get_ip_and_addr() const throw (invalid_argument) {
    struct hostent * he;
...
std::string &
Url::ip() const throw (invalid_argument) {
    try {
        if (ip_ == "")
            get_ip_and_addr();
    } catch (invalid_argument & ia) {
...
struct in_addr &
Url::addr() const throw (invalid_argument) {
    try {
        if (ip_ == "")
            get_ip_and_addr();
    } catch (invalid_argument & ia) {
...
Url::Url(const string & url) throw (invalid_argument, bad_alloc)
    : orig_(url), ip_("")
{
    addr_.s_addr=0;
    try {
